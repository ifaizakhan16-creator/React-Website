          ...
          {history.length > 0 ? history.map((req) => (
            ...
            >
              <div className="flex items-center gap-6">
                ...
                <div className={`w-14 h-14 ${req.color || 'bg-indigo-600'} rounded-2xl flex items-center justify-center text-white font-black`}>
                  {(req.name?.charAt(0) || "U")}
                </div>
              ...
              <div>
                ...
                {req.status === 'rejected' ? (
                  ...
                  onClick={() => onRestore(req._id)}
                    className="px-10 py-5 bg-rose-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-600 transition-all"
                  >
                    Restore Request
                  </button>
                ) : (
                  <Link to="/chatroom">
                    <button className="px-10 py-5 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl">
                      Start Communication
                    </button>
                  </Link>
                )}
            ...
            <div className="text-center py-32 bg-slate-50 rounded-[4rem] border-2 border-dashed border-slate-200">
              <p className="text-slate-400 font-black uppercase tracking-widest text-[10px]">
                No transaction history. Accept or reject an inbox proposal first.
              </p>
            </div>
          )}
